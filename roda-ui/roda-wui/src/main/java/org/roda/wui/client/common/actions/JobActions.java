...
public class JobActions extends AbstractActionable<Job> {
    ...
    Dialogs.showConfirmDialog(messages.jobStopConfirmDialogTitle(), messages.jobStopConfirmDialogMessage(),
      messages.dialogCancel(), messages.dialogYes(), new ActionAsyncCallback<Boolean>(callback) {
        ...
        public void onSuccess(Boolean confirmed) {
          if (confirmed) {
            // FIXME 20180731 bferreira: not calling the callback on this case results in
            // the stop button staying disabled, find some better way to do this and update
            // this logic
  ...
  @Override
  public ActionsBundle<Job> createActionsBundle() {
    ActionsBundle<Job> jobActionableBundle = new ActionsBundle<>();

    // MANAGEMENT
    ActionsGroup<Job> managementGroup = new ActionsGroup<>(messages.sidebarActionsTitle());
